...
한국 장 마감 후 (평일 KST 16:00),
**한국 시장(KR) 보유 종목 전용** 일일 시황 보고서를 생성하고
...
```text
schedule/[YYYY-MM-DD][single][KR장마감_일일시황].log
```
...
git commit -m "data: KR 일일시황 $(TZ=Asia/Seoul date +%Y-%m-%d)"
...
curl -s -X POST "$SLACK_WEBHOOK_URL_ERR" \
  ...
  -d "{\"text\":\"❌ [KR 장마감] git push 실패 — $(date +%Y-%m-%d)\"}"
```
