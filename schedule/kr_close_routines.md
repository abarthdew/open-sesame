## KR Market Close Daily Report

You are an automated agent for the `open-sesame` investment analysis system.

After the Korean market close (weekdays 16:00 KST),
generate a daily market report **for Korean market (KR) holdings only**,
...
for idx_code, name in [('1001','KOSPI'),('2001','KOSDAQ')]:
    try:
        df = stock.get_index_ohlcv_by_date(start, end, idx_code)
        if not df.empty:
            p, p1 = df['종가'].iloc[-1], df['종가'].iloc[-2]
            print(f'{name}: {p:,.2f}pt ({(p/p1-1)*100:+.2f}%)')
...
```
KOSPI market news [YYYY-MM-DD]
Samsung Electronics stock news [YYYY-MM-DD]
SK hynix stock news [YYYY-MM-DD]
```
...
```text
schedule/[YYYY-MM-DD][single][KR-close-daily].log
```
...
- KOSPI / KOSDAQ index levels and today's market overview
- holdings table:
  - current price
  - daily change
  ...
  - 20-day return
- sector comments:
  - Semiconductors: Samsung Electronics / SK hynix / KODEX Semiconductor
...
git commit -m "data: KR daily report $(TZ=Asia/Seoul date +%Y-%m-%d)"
...
curl -s -X POST "$SLACK_WEBHOOK_URL_ERR" \
  ...
  -d "{\"text\":\"❌ [KR Market Close] git push failed — $(date +%Y-%m-%d)\"}"
```
